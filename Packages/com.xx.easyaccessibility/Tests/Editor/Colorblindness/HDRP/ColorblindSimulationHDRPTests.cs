...
{
    public class ColorblindSimulationHDRPTests
    {
        ColorblindSimulationHDRP m_component;
        ...
        static readonly MethodInfo k_Render =
            typeof(ColorblindSimulationHDRP).GetMethod("Render", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);

        [SetUp]
        public void SetUp()
        {
            m_cleanup = new List<Object>();
            m_component = ScriptableObject.CreateInstance<ColorblindSimulationHDRP>();
        ...
        Material MakeMaterial() => Track(new Material(Shader.Find("Shader Graphs/ColorblindSimulation_HDRP")));
